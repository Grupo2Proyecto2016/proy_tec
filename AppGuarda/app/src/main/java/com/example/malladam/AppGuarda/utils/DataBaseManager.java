...
import com.example.malladam.AppGuarda.utils.DataBaseHelper;
...
public class DataBaseManager {
    ...
    public static final String TABLE_NAME_LOGICAPARADAS = "logicaParadas";
    ...
    public static final String CN_ID_ULTIMA_PARADA= "id_ult_parada";
    public static final String CN_ID_PROXIMA_PARADA = "id_prox_parada";
    public static final String CN_DIST_PROXIMA_PARADA = "dist_prox_parada";
    ...
    public static final String CREATE_TABLE_LOGICAPARADAS = "create table " +TABLE_NAME_LOGICAPARADAS+ " ("
            + CN_ID_ULTIMA_PARADA + " integer not null,"
            + CN_ID_PROXIMA_PARADA + " integer not null,"
            + CN_DIST_PROXIMA_PARADA + " real);";
    ...
    public boolean liberanAsientosByParadaDestino(int id_paradaDestino){
        Boolean bajaAlguien = false;
        String whereClause = CN_IDPARADABAJA_ASIENTO+" = ? ";
        String[] whereArgs = new String[] { String.valueOf(id_paradaDestino) };
        Cursor resultado = db.query(TABLE_NAME_ASIENTOS, null, whereClause, whereArgs, null, null, null);
    ...
    public void eliminarAsientosByParadaDestino(int id_paradaDestino){
        db.delete(TABLE_NAME_ASIENTOS, CN_IDPARADABAJA_ASIENTO+"="+String.valueOf(id_paradaDestino), null);
    }
    ...
    public Parada getParadaDelViajeById(int idParada){

        Cursor resultado = null;
        Parada parada = new Parada();
        try{
            resultado = db.rawQuery("SELECT * FROM "+TABLE_NAME_PARADAS+" WHERE "+CN_IDPARADA_PARADAS+"=?", new String[] {idParada + ""});
    ...
    public void insertarLogicaParadas(int idUltParada, int idProxParada, float distProxParada){

        eliminarLogicaParadas();

        ContentValues valores = new ContentValues();
        valores.put(CN_ID_ULTIMA_PARADA, idUltParada);
        valores.put(CN_ID_PROXIMA_PARADA, idProxParada);
        valores.put(CN_DIST_PROXIMA_PARADA, distProxParada);

        db.insert(TABLE_NAME_LOGICAPARADAS, null, valores);

    }


    public int getIdUltParada(){
        Cursor resultado = db.query(TABLE_NAME_LOGICAPARADAS, null, null, null, null, null, null);
        if (resultado.moveToFirst()){
            return resultado.getInt(resultado.getColumnIndex(CN_ID_ULTIMA_PARADA));
        }else return 0;
    }

    public int getIdProxParada(){
        Cursor resultado = db.query(TABLE_NAME_LOGICAPARADAS, null, null, null, null, null, null);
        if (resultado.moveToFirst()){
            return resultado.getInt(resultado.getColumnIndex(CN_ID_PROXIMA_PARADA));
        }else return 0;
    }

    public double getDistProxParada(){
        Cursor resultado = db.query(TABLE_NAME_LOGICAPARADAS, null, null, null, null, null, null);
        if (resultado.moveToFirst()){
            return resultado.getDouble(resultado.getColumnIndex(CN_DIST_PROXIMA_PARADA));
        }else return 0;
    }

    public void setDistProxParada(double distProxParada) {
        ContentValues cv = new ContentValues();
        cv.put(CN_DIST_PROXIMA_PARADA, distProxParada);
        db.update(TABLE_NAME_LOGICAPARADAS, cv, null, null);
    }

    public void setIdUltParada(int idUltParada) {
        ContentValues cv = new ContentValues();
        cv.put(CN_ID_ULTIMA_PARADA, idUltParada);
        db.update(TABLE_NAME_LOGICAPARADAS, cv, null, null);
    }

    public void setIdProxParada(int idProxParada) {
        ContentValues cv = new ContentValues();
        cv.put(CN_ID_PROXIMA_PARADA, idProxParada);
        db.update(TABLE_NAME_LOGICAPARADAS, cv, null, null);
    }

    public void setParadas(int id_ultimaParada, int id_proximaParada) {
        ContentValues cv = new ContentValues();
        cv.put(CN_ID_ULTIMA_PARADA, id_ultimaParada);
        cv.put(CN_ID_PROXIMA_PARADA, id_proximaParada);
        db.update(TABLE_NAME_LOGICAPARADAS, cv, null, null);
    }

    public void eliminarLogicaParadas(){
        db.delete(TABLE_NAME_LOGICAPARADAS,null,null);
    }
